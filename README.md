...
- **Resource launchpad** — Track and open top recommended artifacts with per-mode completion progress, validated single/bulk open actions, pop-up blocker feedback, resilient local-state parsing, truthy-only normalized state persistence, accurate progress updates after bulk opens, and loop-safe cross-panel sync events.
