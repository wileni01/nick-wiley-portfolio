...
- **Chat API hardening** — Chat requests use schema validation, provider allowlisting, bounded message windows, sanitized inputs/context, explicit user-turn requirement for query grounding, rate limiting, and shared automatic provider fallback when only one valid API key is configured (trimmed + minimum-length checked), with provider-request/selection headers for observability.
