...
- **Deterministic recommendation/API utility tests** — `npm test` runs Node-based TypeScript tests that verify adaptive recommendation determinism, ranking bounds/sorting/diversity guarantees, safe internal URL outputs, deterministic narrative fallbacks for invalid mode selections, hardened JSON request/response behavior (BOM tolerance, content-type/content-length guards, immutable security headers), request-IP normalization across proxy header formats, and rate-limit/request-id normalization guarantees used by shared API response/context helpers.
