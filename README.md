...
- **Contact API hardening** — Contact submissions use schema validation, honeypot filtering, rate limiting, input sanitization, minimal metadata logging, and optional Resend delivery with failure handling.
...
- **API IP normalization** — Server routes normalize `x-forwarded-for`, standardized `forwarded`, and `x-real-ip` headers (including port/zone-id stripping), and now discard non-IP tokens before rate-limit keying to reduce malformed or high-cardinality edge cases, with lazy in-memory rate-limit cleanup.
- **Shared API JSON utilities** — Server routes reuse shared JSON request parsing/response helpers for consistent invalid-payload handling, including empty-body checks, JSON content-type enforcement, and payload-size guard responses (with content-length prechecks).
- **Shared API request context** — API routes reuse shared request context helpers for consistent request IDs, IP-based rate-limit keying, and response header construction.
