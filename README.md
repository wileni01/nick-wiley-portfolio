...
- **Shareable prep links** — Copy mode links that preserve company, persona, provider, focus note, and interview date context.
...
- **Interview date tracker** — Set a per-mode interview date (with quick-set controls) to anchor urgency, include date context in exported briefs, launch Google Calendar interview/prep checkpoint events, and download an `.ics` prep plan.
