...
- **Shared API request context** — API routes reuse shared request context helpers for consistent request IDs (UUID with robust random-token fallback), IP-based rate-limit keying, normalized rate-limit config usage, and response header construction.
