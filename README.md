...
- **External-link safety** — Adaptive popup-launch actions now use a shared opener that validates HTTP(S) URLs and reports blocked pop-up outcomes.
...
- **Resource launchpad** — Track and open top recommended artifacts with per-mode completion progress, validated single/bulk open actions, and pop-up blocker feedback.
