...
- **Contact API hardening** — Contact submissions use schema validation, honeypot filtering, rate limiting, input sanitization, minimal metadata-only logging (redacted sender + field lengths, no raw subject/body previews), and optional Resend delivery with bounded/redacted provider-error handling.
