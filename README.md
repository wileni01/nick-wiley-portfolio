...
- **Interview-mode API hardening** — Interview briefing requests/responses use schema validation, allowlisted IDs/providers, bounded/sanitized narrative fields, shared provider fallback routing when available, deterministic fallback behavior (with structured warning diagnostics on AI-generation fallback and explicit `X-AI-Narrative-Fallback` reason headers), and provider-request/selection headers.
