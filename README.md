...
- **Deterministic recommendation/API utility tests** — `npm test` runs Node-based TypeScript tests that verify adaptive recommendation determinism, ranking bounds/sorting/diversity guarantees, safe internal URL outputs, deterministic narrative fallbacks for invalid mode selections, adaptive state/history sanitization guards (boolean/focus/prep history + prep bundle/mock-session parsing), hardened JSON request/response behavior (BOM tolerance, content-type/content-length guards, immutable security headers), request-IP normalization across proxy header formats, rate-limit/request-id normalization guarantees used by shared API response/context helpers, structured server-log sanitization/redaction behavior, AI-provider resolution/fallback header semantics, contact-delivery payload/error redaction safeguards, and route-level fallback telemetry headers for invalid/rate-limited chat/contact/interview-mode requests (including provider fallback header states on deterministic error paths).
