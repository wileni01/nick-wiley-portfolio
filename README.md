...
- **API IP normalization** — Server routes normalize `x-forwarded-for`, standardized `forwarded`, and `x-real-ip` headers (including port/zone-id stripping) before rate-limit keying to reduce malformed or high-cardinality edge cases, with lazy in-memory rate-limit cleanup.
