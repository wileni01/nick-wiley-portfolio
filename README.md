...
- **Shared API JSON utilities** — Server routes reuse shared JSON request parsing/response helpers for consistent invalid-payload handling, including empty-body checks, strict JSON media-type validation, validated content-length parsing + payload-size guard responses (with UTF-8 post-read size validation), and resilient response serialization fallback for unexpected payload shapes (with non-error responses safely promoted to 500 on serialization failure).
